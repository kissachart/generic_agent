...
//Part 4 select agent and save for check timeout
        async function getagents() {
            ...
            try{
                agents= await database.query("select * from gseven.agents where CONTROL in (1,2)");
                ...
                write_log(1,"Can't get agent from database gseven.agents from database ip :"+IPAddress_database+". Program will retry in 1 secound.");
                setTimeout(()=>{
                    getagents();
                }, 1000);
            }
...
async function loop_main() {
	...
	while (qFront!=qRear) {
    	...
    	//
        for (var i=0;i<9;i++){ fields[i]=(fields[i]==undefined)?"":fields[i];}

        try{
            write_log(2,"call incoming ["+qRear+"]- call gseven.incoming_event_handler('"+
            fields[0]+"','"+
            fields[1]+"','"+
            fields[2]+"','"+
            fields[3]+"','"+
            ...
            fields[7]+"'"+
            ");");
            var query= await database.query("call gseven.incoming_event_handler('"+
                                                                        fields[0]+"','"+
                                                                        fields[1]+"','"+
                                                                        fields[2]+"','"+
                                                                        fields[3]+"','"+
            ...
            if (GSAM_message != undefined){
                ...
                write_log(2,"incoming match!!! ["+qRear+"]    - call gseven.incoming_event_handler('"+
                                                                                                fields[0]+"','"+
                                                                                                fields[1]+"','"+
                                                                                                fields[2]+"','"+
                                                                                                fields[3]+"','"+
                ...
                write_log(2,"incoming not match!!! ["+qRear+"]- call gseven.incoming_event_handler('"+
                                                                                                    fields[0]+"','"+
                                                                                                    fields[1]+"','"+
                                                                                                    fields[2]+"','"+
                                                                                                    fields[3]+"','"+
	...
	Object.keys(agents_status).forEach(async function _(key){
		...
		if (Math.round((d-agents_status[key].STAMPTIME)/1000)>=agents_status[key].AGENT_TIMEOUT*60) {
            ...
            if (agents_status[key].STAMPTIME.getTime() === agents_status[key].FIRST_STAMPTIME.getTime()){
                ...
                var GSAM_message = undefined;
                try{
                    write_log(2,"call gseven.incoming_event_handler('"+
                                                                        "Generic','"+
                                                                        datetime+"','"+
                                                                        "','"+
                                                                        key+"','"+
...
//Part write log
        function write_log(system,text) {
            ...
            if (system == "1"||system == "system"){
                ...
                LogFileName = global.agent_name+"other"+year+"-"+month+"-"+date;
